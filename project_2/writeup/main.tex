...
\section*{Conclusion}
This implementation of Minimax showed how well adversarial search can be used to create a strong AI player for an application like Connect 4. The optimization techniques that were implemented also proved to be effective in moves explored as perdicted by their theoretical $O(time)$ efficiency calculations. For example, comparing the base Minimax $depth = 4$ first move search of 2800 moves, which almost exactly the sum of all 4 depths of the tree ($7^0 + 7^1 +7^2 +7^3 + 7^4$) to the Minimax $depth = 4$ with Alpha-Beta pruning first move search of 857 moves, we can see that the number of nodes explored is reduced by a factor of about 3.27, which is consistent with the theoretical improvement from $O(b^d)$ to ~$O(b^{3d/4})$. Comparing those first two rows of the base Minimax and the Minimax with Alpha-Beta pruning is interesting as all 3 outcomes at the different depths are the same, but the number of nodes or the efficiency of the search is significantly improved with the pruning technique, this is what we expected. We also would have expected the third row to match these three outcomes as well, but due to the fact that the Move Re-Ordering is a static center sorted order as opposed to the first two rows using the left to right static sorting of move exploration per layer, this actually changes which decisions are being made. The way this changes decision making is because the program does nothing other than take the first move in the list currently when it comes to tie breaking. So in the first two rows of the table the left most move is always explored first and therefore chosen in the case of a tie, but in the third row the center most move is explored first and therefore chosen in the case of a tie. Thus explaining that bit of data I at first was not expecting. Currently in this implementation using center static Move Re-Ordering paired with Alpha-Beta pruning is the most efficient of all the tested methods. This is due to the center moves being better Connect 4 moves is a very strong general heuristic. The Iterative Deepening paired with Dynamic Move Re-Ordering that starts out as the central move ordering could be improved on by the move ordering being calculated at each layer of the tree, as opposed to once at the beginning of each iterative deepening call to Minimax. With this improvement its efficiency would likely surpass the static center Move Re-Ordering + Alpha-Beta pruning. The Selective Deepening optimization did in fact make the Connect 4 AI a better player. It could be improved on by allowing the search to coninue extending the search of a threat path until it was no longer a thread, as opposed to the single layer extension that was implemented in this project.

\section*{Use of AI}
\subsection*{Code Generation}
The Claude Sonnet 4.6 Model was used for the initial implementation of the projects argument parsing, Board class, and the visual library set up of the realtime Connect 4 board. This was done by providing the model with a detailed root level .md file and a few specific .md task files. Other than that the majority of the code was written by hand to better understand the Minimax algorith and its optimization techniques.
\subsection*{Writeup}
The Claude Sonnet 4.6 Model was used for all the Latex structual code and library inclusions needed for this document. The content of each section of the document was written by hand and are my own words. The model was used to help with the formatting of the tables and figures, but the data content of the tables were gathered manually by running the program and recording the data. The model was also used for grammar and spelling checks of the section content.
\end{document}
